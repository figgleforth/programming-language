...
- The overlap form (`by N,overlap`) computes `step = stride - overlap`, then builds windows with `(0...values_array.length).step(step).map { |i| values_array[i, stride] }`. Every window is kept, including a trailing one shorter than `stride` (`chunk.length <= stride`, always true) — same as the plain no-overlap chunking above, which keeps a final undersized chunk through Ruby's `each_slice`. Since only a window's first element is guaranteed to exist, a loop body indexing past `it.0` should use `.?N` (returns `nil` past the end) rather than `.N` (raises `Code::Invalid_Array_Index`). `overlap` must be a smaller integer than `stride`. `Code::For_Loop_Expr#overlap` (`source/proxies/expressions.rb`) holds the parsed value; `interp_for_loop` (`interpreter.rb`) validates and applies it.
