...
# Runs every guides/*.code file end to end and asserts none of them raise -- these are meant to be
# runnable teaching examples (see .claude/CLAUDE.md's guides/ doc style notes), so a file that
...
class Examples_Test < Base_Test
	::Dir.glob(::File.join(__dir__, '../guides/*.code')).sort.each do |filepath|
