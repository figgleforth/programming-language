...
How about an interesting Binary_Expr operator overloading example? Adding two Boos together produces a Super_Boo. Why am I excited about this? 🤓
```
Super_Boo {
	boo1 =;
	boo2 =;
...
This just opened my eyes to using Refs to reference other instances and not just Block_Exprs. I guess this is a good part of the language to decide when/where/how to pass by value versus reference.

Inline composition is a reference, so that allows changing live data.

Should assigning Instances to a variable be reference? In Ruby assignments are by reference. In Ruby, functions do not have access to variables outside of it, they have stack visibility like my Instance scope. Hmmm...

Param by reference and composition
	funk { &comp, other -> }
	funk { & comp, other -> } 
Modifying comp also modifies the variable passed into funk()

Param by reference
	funk { ^other -> }
	funk { ^ other -> }
Modifying other also modifies the variable passed into funk()

Param by value
	funk { other -> }
Funk receives a copy of other
