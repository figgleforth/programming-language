...
module Code
	class Interpreter
		...
		class << self
			...
			# ruby/shared/cached_by_path.rb), so this just delegates to each one's own generic reset.
		...
		def initialize
			...
			# Interpreter, so the token the /_lang/live-reload stream reports changes, which is exactly
		...
		def run source_code
			...
			if @stack.empty?
				# todo; Global should be created by interping lang/global.code, which is what I want to rename lang/global.code to
				global  = Global.new
		...
		#
		# An `element_key` (the element's own `key := '...'`, see lang/html.code) pins the token by
		...
		def start_server server
			...
			# Live reload event stream (see ruby/shared/live_reload.js). Mounted only under Hot_Reloader,
			...
			# response access.
			if live_reload
				webrick.mount_proc '/_lang/live-reload' do |_req, res|
					res.status           = 200
			...
			if route_function
				...
				begin
					...
					if response.body.to_s =~ /<html|<body|<head/i
						...
						dom_js              = self.class.cached_asset_source_by_path['ruby/shared/dom.js'] ||= ::File.read('ruby/shared/dom.js')
						script_tag          = "<script>#{dom_js}</script>"
						view_transition_css = self.class.cached_asset_source_by_path['ruby/shared/view_transition.css'] ||= ::File.read('ruby/shared/view_transition.css')
						view_transition_tag = "<style>#{view_transition_css}</style>"

						# Only present under Hot_Reloader -- pairs with the /_lang/live-reload endpoint.
						live_reload_tag = if live_reload
							lr_js = self.class.cached_asset_source_by_path['ruby/shared/live_reload.js'] ||= ::File.read('ruby/shared/live_reload.js')
							"<script>#{lr_js}</script>"
						else
							''
						end
		...
		# The literal `Any` type (lang/global.code), a universal wildcard -- see #interp_comparison_infix.
		def any_type? value
		...
		def interp_fence expr
			...
			#
			# `Fence | String {}` (lang/fence.code, loaded by lang/global.code) is the real declared
		...
		# What a variadic param binds -- a Code::Array linked to the `Arguments` type (see lang/array.code).
		...
		# A value built directly from a string literal gets wrapped into a real Code::String carrying the literal's own `quotation_style`, instead of staying the bare Ruby string #interp_string normally returns. Struct/Member's to_s(;) (lang/member.code) and Array/Dictionary/Tuple's to_s(;) (lang/array.code, lang/dictionary.code, lang/global.code) read `.quotation_style` straight off the value to decide how to quote it for display.
		...
		def build_struct names, type_names, types, values
			...
			# is loaded (`lang/struct.code`). The plain quartet (`@names`/`@type_names`/`@type_objects`/
