module Code
	...
	class Instance < Type
		...
		def []= key, value
			...
			if delegate_name && key.to_s == delegate_name
				...
				# coerce (Code::Integer#value= runs to_i, Code::Decimal#value= builds a BigDecimal).
	...
	class Nil < Instance # Like Ruby's NilClass, this represents the absence of a value. An ordinary instance -- constructible like any other type, so `Nil()` / a tagged `Nil\Error()` work and Code code can hold its own shared nil.
	end
	...
	# Code::Range is an Instance wrapping a Ruby ::Range -- see ruby/proxies/range.rb + lang/range.code.
