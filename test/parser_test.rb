require 'minitest/autorun'
require_relative '../lib/air'
require_relative 'base_test'

class Parser_Test < Base_Test
	def test_identifiers
		zipped = %w(variable_or_function CONSTANT Type).zip %I(identifier IDENTIFIER Identifier)
		zipped.each do |code, type|
			out = Air.parse code
			assert_kind_of Air::Identifier_Expr, out.first
			assert_equal code, out.first.value
			assert_nil out.first.type
		end
	end

	def test_integers_and_floats
		out = Air.parse '4'
		assert_kind_of Air::Number_Expr, out.first
		assert_equal 4, out.first.value
		assert_equal :integer, out.first.type

		out = Air.parse '2.3'
		assert_kind_of Air::Number_Expr, out.first
		assert_equal 2.3, out.first.value
		assert_equal :float, out.first.type
	end

	def test_numbers_with_prefixes
		out = Air.parse '-42'
		assert_kind_of Air::Prefix_Expr, out.first
		assert_equal '-', out.first.operator
		assert_equal 42, out.first.expression.value
		assert_kind_of Air::Number_Expr, out.first.expression

		out = Air.parse '+4.2'
		assert_kind_of Air::Prefix_Expr, out.first
		assert_equal '+', out.first.operator
		assert_equal 4.2, out.first.expression.value
		assert_kind_of Air::Number_Expr, out.first.expression
	end

	def test_numbers_with_underscores
		out = Air.parse '2_000'
		assert_equal 1, out.count
		assert_kind_of Air::Number_Expr, out.first
		assert_equal 2000, out.first.value

		out = Air.parse '3_0_'
		assert_equal 2, out.count
		assert_kind_of Air::Number_Expr, out.first
		assert_equal 30, out.first.value
		assert_kind_of Air::Identifier_Expr, out.last
		assert_equal '_', out.last.value

		out = Air.parse '_2_00'
		assert_equal 1, out.count
		refute_kind_of Air::Number_Expr, out.first
		refute_equal 200, out.first.value

		out = Air.parse '-20three'
		assert_equal 2, out.count
		assert_kind_of Air::Prefix_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.last
		assert_equal 20, out.first.expression.value
		assert_equal 'three', out.last.value

		out = Air.parse '40_two'
		assert_equal 2, out.count
		assert_kind_of Air::Number_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.last
		assert_equal 40, out.first.value
		assert_equal '_two', out.last.value

		out = Air.parse '4__5__2__2'
		assert_equal 2, out.count
		assert_kind_of Air::Number_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.last
		assert_equal 4, out.first.value
		assert_equal '__5__2__2', out.last.value

		out = Air.parse 'a1234'
		assert_equal 1, out.count
		assert_kind_of Air::Identifier_Expr, out.first
		assert_equal 'a1234', out.first.value
		refute out.first.type
	end

	def test_strings
		out = Air.parse '"A string"'
		assert_kind_of Air::String_Expr, out.first
		refute out.first.interpolated

		out = Air.parse "'Another string'"
		assert_kind_of Air::String_Expr, out.first
		refute out.first.interpolated

		out = Air.parse '"An |interpolated| string"'
		assert_kind_of Air::String_Expr, out.first
		assert out.first.interpolated

		out = Air.parse "'Another |interpolated| string'"
		assert_kind_of Air::String_Expr, out.first
		assert out.first.interpolated
	end

	def test_compound_assignments
		out = Air.parse 'numbers += 1623'
		refute_kind_of Air::Identifier_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 1, out.count

		out = Air.parse 'numbers -= 1623'
		assert_kind_of Air::Infix_Expr, out.first

		out = Air.parse 'flag |= 2'
		assert_kind_of Air::Infix_Expr, out.first
	end

	def test_operator_precedence
		out = Air.parse '1 + 2 * 3 / 4 - 5 % 6'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.first.left
		assert_kind_of Air::Number_Expr, out.first.left.left
		assert_equal 1, out.first.left.left.value
		assert_equal '+', out.first.left.operator
		assert_kind_of Air::Infix_Expr, out.first.left.right
		assert_kind_of Air::Infix_Expr, out.first.left.right.left
		assert_kind_of Air::Number_Expr, out.first.left.right.left.left
		assert_equal 2, out.first.left.right.left.left.value
		assert_kind_of Air::Number_Expr, out.first.left.right.left.right
		assert_equal 3, out.first.left.right.left.right.value
		assert_equal '/', out.first.left.right.operator
		assert_kind_of Air::Number_Expr, out.first.left.right.right
		assert_equal 4, out.first.left.right.right.value
		assert_equal '-', out.first.operator
		assert_kind_of Air::Infix_Expr, out.first.right
		assert_kind_of Air::Number_Expr, out.first.right.left
		assert_equal 5, out.first.right.left.value
		assert_equal '%', out.first.right.operator
		assert_kind_of Air::Number_Expr, out.first.right.right
		assert_equal 6, out.first.right.right.value
	end

	def test_operator_precedence_with_parentheses
		out = Air.parse '1 + ((2*3) / 4) - (5 % 6)'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.first.left
		assert_equal '+', out.first.left.operator
		assert_kind_of Air::Number_Expr, out.first.left.left
		assert_equal 1, out.first.left.left.value
		assert_kind_of Air::Circumfix_Expr, out.first.left.right
		assert_kind_of Air::Infix_Expr, out.first.left.right.expressions.first
		assert_equal '/', out.first.left.right.expressions.first.operator
		assert_kind_of Air::Circumfix_Expr, out.first.left.right.expressions.first.left
		assert_equal 2, out.first.left.right.expressions.first.left.expressions.first.left.value
		assert_equal '*', out.first.left.right.expressions.first.left.expressions.first.operator
		assert_equal 3, out.first.left.right.expressions.first.left.expressions.first.right.value
		assert_kind_of Air::Number_Expr, out.first.left.right.expressions.first.right
		assert_equal 4, out.first.left.right.expressions.first.right.value
		assert_equal '-', out.first.operator
		assert_kind_of Air::Circumfix_Expr, out.first.right
		assert_kind_of Air::Number_Expr, out.first.right.expressions.first.left
		assert_equal 5, out.first.right.expressions.first.left.value
		assert_equal '%', out.first.right.expressions.first.operator
		assert_kind_of Air::Number_Expr, out.first.right.expressions.first.right
		assert_equal 6, out.first.right.expressions.first.right.value
	end

	def test_other
		out = Air.parse 'numbers = 4815'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 1, out.count

		out = Air.parse 'numbers;'
		assert_kind_of Air::Postfix_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.first.expression
		assert_equal ';', out.first.operator

		out = Air.parse 'Type = {}'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.first.left
		assert_kind_of Air::Circumfix_Expr, out.first.right
		assert_equal 1, out.count

		out = Air.parse 'time: Float'
		assert_equal 'Float', out.first.type.value

		out = Air.parse 'num: Int = 1 + 2'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.first.right
		assert_equal 'Int', out.first.left.type.value
	end

	def test_more_fixities
		out = Air.parse '1 + 2 * 3 / 4'
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal 1, out.count

		out = Air.parse '1 < 2'
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal 1, out.count

		out = Air.parse '2 >= 1'
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal 1, out.count

		out = Air.parse '1 != 2'
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal 1, out.count

		out = Air.parse '1 == 2'
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal 1, out.count

		out = Air.parse '1 < 2, 4 > 3'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.last
		assert_equal 2, out.count
	end

	def test_ranges
		out = Air.parse '1..2'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Number_Expr, out.first.left
		assert_equal '..', out.first.operator
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 1, out.first.left.value
		assert_equal 2, out.first.right.value

		out = Air.parse '3.0..4.0'
		assert_kind_of Air::Number_Expr, out.first.left
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal '..', out.first.operator
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 3.0, out.first.left.value
		assert_equal 4.0, out.first.right.value

		out = Air.parse '3.<4'
		assert_kind_of Air::Number_Expr, out.first.left
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal '.<', out.first.operator
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 3, out.first.left.value
		assert_equal 4, out.first.right.value

		out = Air.parse '5>.6'
		assert_kind_of Air::Number_Expr, out.first.left
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal '>.', out.first.operator
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 5, out.first.left.value
		assert_equal 6, out.first.right.value

		out = Air.parse '7><8'
		assert_kind_of Air::Number_Expr, out.first.left
		assert_kind_of Air::Infix_Expr, out.first
		assert_equal '><', out.first.operator
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 7, out.first.left.value
		assert_equal 8, out.first.right.value

		out = Air.parse '1..2, 3.<4, 5>.6, 7><8'
		assert_equal 4, out.count
		out.each do
			assert_kind_of Air::Infix_Expr, it
			assert_kind_of Air::Number_Expr, it.left
			assert_kind_of Air::Number_Expr, it.right
		end
	end

	def test_comma_separated_expressions
		out = Air.parse 'a, B, 5, "cool"'
		assert_equal 4, out.count
		assert_kind_of Air::Identifier_Expr, out[0]
		assert_kind_of Air::Identifier_Expr, out[1]
		assert_kind_of Air::Number_Expr, out[2]
		assert_kind_of Air::String_Expr, out[3]
	end

	def test_scope_operators
		out = Air.parse './this_instance'
		assert_kind_of Air::Identifier_Expr, out.first
		assert_equal './', out.first.scope_operator

		out = Air.parse '../global_scope'
		assert_kind_of Air::Identifier_Expr, out.first
		assert_equal '../', out.first.scope_operator

		out = Air.parse '.../third_party'
		assert_kind_of Air::Identifier_Expr, out.first
		assert_equal '.../', out.first.scope_operator
	end

	def test_functions
		out = Air.parse '{;}'
		assert_kind_of Air::Func_Expr, out.first
		assert_empty out.first.expressions
		refute out.first.name

		out = Air.parse '{;
		}'
		assert_empty out.first.expressions
		refute out.first.name

		out = Air.parse 'named_function {;}'
		assert_equal 'named_function', out.first.name.value
	end

	def test_function_params
		out = Air.parse '{ with_param; }'
		assert_equal 1, out.first.expressions.count
		out.first.expressions.each do
			assert_kind_of Air::Param_Expr, it
		end

		out = Air.parse 'named { with_param; }'
		assert_equal 'named', out.first.name.value
		assert_equal 1, out.first.expressions.count
		out.first.expressions.each do
			assert_kind_of Air::Param_Expr, it
		end
		refute out.first.expressions.first.label
		refute out.first.expressions.first.default
		refute out.first.expressions.first.type

		out = Air.parse '{ labeled param; }'
		assert_kind_of Air::Param_Expr, out.first.expressions.first
		assert_equal 'labeled', out.first.expressions.first.label
		assert out.first.expressions.first.label
		refute out.first.expressions.first.default
		refute out.first.expressions.first.type

		out = Air.parse '{ default_values = 4; }'
		assert_kind_of Air::Param_Expr, out.first.expressions.first
		assert out.first.expressions.first.default
		assert_kind_of Air::Number_Expr, out.first.expressions.first.default

		out = Air.parse 'named { and_labeled with_default = 8; }'
		assert_kind_of Air::Param_Expr, out.first.expressions.first
		assert_equal 'and_labeled', out.first.expressions.first.label
		assert_equal 'with_default', out.first.expressions.first.name
		assert_equal 'named', out.first.name.value

		out = Air.parse 'named { with, multiple, even labeled = 4, params = 5; }'
		assert_equal 4, out.first.expressions.count
		assert_equal out.first.expressions.map(&:label), [nil, nil, 'even', nil]
		assert_equal out.first.expressions.map(&:name), %w(with multiple labeled params)
		assert_equal out.first.expressions.map(&:default).map(&:nil?), [true, true, false, false]
	end

	def test_function_bodies
		out = Air.parse '
		square { input;
			input * input
		}'
		refute_empty out.first.expressions
		assert_kind_of Air::Infix_Expr, out.first.expressions[1]

		out = Air.parse '
		nothing { input;
			return input
		}'
		assert_kind_of Air::Prefix_Expr, out.first.expressions[1]
		assert_kind_of Air::Identifier_Expr, out.first.expressions[1].expression
	end

	def test_function_signatures
		out = Air.parse 'nothing { input;
			return input
		}'
		assert_equal 'nothing{input;}', out.first.signature
	end

	def test_complex_function
		out = Air.parse '
		curr? { sequence;
			if not remainder or not lexemes?
				return false
			end

			slice = remainder.slice(0, sequence.count)
			slice.{;
				expected = sequence[at]

				if expected === Array
					expected.any? {;
						it == it2
					}
				else
					it == expected
				end
			}
		}'
		assert_kind_of Air::Func_Expr, out.first
		assert_equal 'curr?', out.first.name.value
		assert_equal 4, out.first.expressions.count

		early_return = out.first.expressions[1]
		assert_kind_of Air::Conditional_Expr, early_return
		assert_kind_of Air::Infix_Expr, early_return.condition
		assert_equal 'or', early_return.condition.operator
		assert_kind_of Air::Prefix_Expr, early_return.condition.left
		assert_kind_of Air::Prefix_Expr, early_return.condition.right
		assert_equal 1, early_return.when_true.count # todo One for return and one for false in `return false`. Maybe I should make it a prefix keyword.

		slice = out.first.expressions[2]
		assert_kind_of Air::Infix_Expr, slice

		tap = out.first.expressions.last
		assert_kind_of Air::Infix_Expr, tap
		assert_kind_of Air::Func_Expr, tap.right
		assert_equal 2, tap.right.expressions.count
		assert_kind_of Air::Infix_Expr, tap.right.expressions.first
		assert_kind_of Air::Conditional_Expr, tap.right.expressions.last

		conditional = tap.right.expressions.last
		assert_kind_of Air::Infix_Expr, conditional.condition
		assert_equal '===', conditional.condition.operator
		assert_equal 1, conditional.when_true.count # todo I don't think when_true and when_false convey that they return an array
		assert_equal 1, conditional.when_false.count

		any = conditional.when_true.first
		assert_kind_of Air::Infix_Expr, any
		assert_kind_of Air::Func_Expr, any.right
		assert_kind_of Air::Infix_Expr, any.right.expressions.first
		assert_equal 'it', any.right.expressions.first.left.value
		assert_equal 'it2', any.right.expressions.first.right.value
	end

	def test_function_calls
		out = Air.parse '{;}()'
		assert_kind_of Air::Call_Expr, out.first
		assert_kind_of Air::Func_Expr, out.first.receiver
		assert_empty out.first.arguments

		out = Air.parse '{;}(true)'
		refute_empty out.first.arguments
		assert_kind_of Air::Identifier_Expr, out.first.arguments.first

		out = Air.parse '{;}(1, 2, 3)'
		out.first.arguments.each do
			assert_kind_of Air::Number_Expr, it
		end
	end

	def test_types
		out = Air.parse 'String {}'
		assert_kind_of Air::Type_Expr, out.first
		assert_equal 'String', out.first.name.value

		out = Air.parse 'Transform {
			position;
			rotation;
		}'
		assert_equal 2, out.first.expressions.count

		out = Air.parse 'Entity {
			|Transform
		}'
		assert_kind_of Air::Composition_Expr, out.first.expressions.first
		assert_equal '|', out.first.expressions.first.operator
		assert_equal 'Transform', out.first.expressions.first.identifier.value
	end

	def test_mixed_inline_compositions
		out = Air.parse 'Xform | Transform ~ Vec2 & This ^ That {}'
		assert_kind_of Air::Composition_Expr, out.first.expressions.first
		assert_equal '|', out.first.expressions[0].operator
		assert_equal 'Transform', out.first.expressions[0].identifier.value
		assert_equal '~', out.first.expressions[1].operator
		assert_equal 'Vec2', out.first.expressions[1].identifier.value
		assert_equal '&', out.first.expressions[2].operator
		assert_equal 'This', out.first.expressions[2].identifier.value
		assert_equal '^', out.first.expressions[3].operator
		assert_equal 'That', out.first.expressions[3].identifier.value
	end

	def test_control_flows
		out = Air.parse 'if true
			celebrate()
		end'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Call_Expr, out.first.when_true.first

		out = Air.parse 'wrap { number, limit;
			if number > limit
				number = 0
			end
		 }'
		assert_kind_of Air::Conditional_Expr, out.first.expressions[2]

		out = Air.parse 'if 1 + 2 * 3 == 7
			"This one!"
		elif 1 + 2 * 3 == 9
			\'No, this one!\'
		else
			\'🤯\'
		end'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Conditional_Expr, out.first.when_false
		assert_kind_of Air::String_Expr, out.first.when_false.when_false.first
	end

	def test_conditionals_at_end_of_line
		out = Air.parse 'eat while lexemes? && curr?()'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.first.condition
		assert_kind_of Air::Identifier_Expr, out.first.when_true.first
	end

	def test_unless_conditional
		out = Air.parse 'do_this unless the_condition'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.first.condition
		assert_equal 'unless', out.first.type
		assert_equal 'the_condition', out.first.condition.value
		assert_kind_of Air::Identifier_Expr, out.first.when_false.first
		assert_equal 'do_this', out.first.when_false.first.value
	end

	def test_until_conditional
		out = Air.parse 'repeat_this until the_condition'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.first.condition
		assert_equal 'until', out.first.type
		assert_equal 'the_condition', out.first.condition.value
		assert_kind_of Air::Identifier_Expr, out.first.when_false.first
		assert_equal 'repeat_this', out.first.when_false.first.value
	end

	def test_silly_elwhile
		out        = Air.parse '
		while a
			1
		elwhile b
			2
		elwhile c
			3
		else
			4
		end
		'
		while_case = out.first
		assert_kind_of Air::Conditional_Expr, while_case
		assert_equal 'while', while_case.type
		assert_kind_of Air::Number_Expr, while_case.when_true.first
		assert_equal 1, while_case.when_true.first.value
		assert_kind_of Air::Conditional_Expr, while_case.when_false

		elwhile = while_case.when_false
		assert_equal 'elwhile', elwhile.type
		assert_kind_of Air::Number_Expr, elwhile.when_true.first
		assert_equal 2, elwhile.when_true.first.value
		assert_kind_of Air::Conditional_Expr, elwhile.when_false

		elwhile = elwhile.when_false
		assert_equal 'elwhile', elwhile.type
		assert_kind_of Air::Number_Expr, elwhile.when_true.first
		assert_equal 3, elwhile.when_true.first.value
		assert_kind_of Air::Number_Expr, elwhile.when_false.first
		assert_equal 4, elwhile.when_false.first.value
	end

	def test_if_else
		# Direct copy-past from test_silly_elwhile
		out = Air.parse '
		if a
			1
		elif b
			2
		elif c
			3
		else
			4
		end
		'

		# elif elif else
		if_case = out.first
		assert_kind_of Air::Conditional_Expr, if_case
		assert_equal 'if', if_case.type
		assert_kind_of Air::Number_Expr, if_case.when_true.first
		assert_equal 1, if_case.when_true.first.value
		assert_kind_of Air::Conditional_Expr, if_case.when_false

		elif_case = if_case.when_false
		assert_equal 'elif', elif_case.type
		assert_kind_of Air::Number_Expr, elif_case.when_true.first
		assert_equal 2, elif_case.when_true.first.value
		assert_kind_of Air::Conditional_Expr, elif_case.when_false

		elif_case = elif_case.when_false
		assert_equal 'elif', elif_case.type
		assert_kind_of Air::Number_Expr, elif_case.when_true.first
		assert_equal 3, elif_case.when_true.first.value
		assert_kind_of Air::Number_Expr, elif_case.when_false.first
		assert_equal 4, elif_case.when_false.first.value
	end

	def test_circumfixes
		out = Air.parse '[], (), {}'
		assert_equal 3, out.count
		out.each do |it|
			assert_kind_of Air::Circumfix_Expr, it
			assert_empty it.expressions
		end

		out = Air.parse '[1, 2, 3]'
		assert_equal 3, out.first.expressions.count
	end

	def test_type_init
		out = Air.parse 'Type()'
		assert_kind_of Air::Call_Expr, out.first
	end

	def test_func_call
		out = Air.parse 'funk()'
		assert_kind_of Air::Call_Expr, out.first
	end

	def test_call_expr_improvement
		out = Air.parse 'Some.thing(1)'
		assert_kind_of Air::Call_Expr, out.first
		assert_kind_of Air::Infix_Expr, out.first.receiver
		assert_kind_of Air::Number_Expr, out.first.arguments.first
	end

	def test_return_is_an_identifier
		out = Air.parse 'return 1 + 2'
		assert_kind_of Air::Prefix_Expr, out.first
	end

	def test_return_with_conditional_at_end_of_line
		out = Air.parse 'return x unless y'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Prefix_Expr, out.first.when_false.first
		assert_kind_of Air::Identifier_Expr, out.first.when_false.first.expression
		assert_kind_of Air::Identifier_Expr, out.first.condition
	end

	def test_return_with_conditionals
		out = Air.parse 'return 3 if true'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Prefix_Expr, out.first.when_true.first
		assert_kind_of Air::Number_Expr, out.first.when_true.first.expression
		assert_kind_of Air::Identifier_Expr, out.first.condition
	end

	def test_identifier_dot_integer_is_an_infix
		out = Air.parse 'something.4'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.first.left
		assert_kind_of Air::Number_Expr, out.first.right
		assert_equal 4, out.first.right.value
	end

	def test_identifier_dot_float_is_an_infix
		out = Air.parse 'not_gonna_work.4.2'
		assert_kind_of Air::Infix_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out.first.left
		assert_kind_of Air::Array_Index_Expr, out.first.right
		assert_equal '4.2', out.first.right.value
		assert_equal [4, 2], out.first.right.indices_in_order
	end

	def test_multidot_number_lexeme
		out = Air.parse '4.8.15.16.23.42'
		assert_kind_of Air::Array_Index_Expr, out.first
		assert_equal '4.8.15.16.23.42', out.first.value
		assert_equal [4, 8, 15, 16, 23, 42], out.first.indices_in_order
	end

	def test_complex_return_with_conditionals
		out = Air.parse 'return 4+2 if true'
		assert_kind_of Air::Conditional_Expr, out.first
		assert_kind_of Air::Prefix_Expr, out.first.when_true.first
		assert_kind_of Air::Infix_Expr, out.first.when_true.first.expression
		assert_kind_of Air::Identifier_Expr, out.first.condition
	end

	def test_possibly_ambigous_type_and_func_syntax_mixture
		out = Air.parse 'x ; y ; z'
		assert_kind_of Air::Postfix_Expr, out.first
		assert_kind_of Air::Postfix_Expr, out[1]
		assert_kind_of Air::Identifier_Expr, out.last

		out = Air.parse 'x , y , z'
		assert_kind_of Air::Identifier_Expr, out.first
		assert_kind_of Air::Identifier_Expr, out[1]
		assert_kind_of Air::Identifier_Expr, out.last
	end

	def test_infinite_loop_bug
		out = Air.parse 'Identifier {;}'
		assert_kind_of Air::Type_Expr, out.first

		out = Air.parse 'x; , y; , z;'
		assert_kind_of Air::Postfix_Expr, out.first
		assert_kind_of Air::Postfix_Expr, out[1]
		assert_kind_of Air::Postfix_Expr, out.last
	end

	def test_double_less_than_is_operator
		out = Air.parse '<<'
		assert_kind_of Air::Operator_Expr, out.first
	end

	def test_reference_decorator_on_identifier_expr
		out = Air.parse '@count'
		assert out.first.reference

		out = Air.parse 'count'
		refute out.first.reference
	end

	# TODO :incomplete
	def test_for_loops
		out = Air.parse '
		for []
		end'
		assert_empty out.first.body
		assert_instance_of Air::Circumfix_Expr, out.first.iterable # note, The iterable becomes an Array in the interpreter.
		assert_equal '[]', out.first.iterable.grouping
	end

	def test_import_syntax
		out = Air.parse 'project_root = ~/'
	end

	def test_directive_identifier
		out = Air.parse '#whatever'
		refute_instance_of Air::Directive_Expr, out.first

		out = Air.parse '#whatever(a, b)'
		assert_instance_of Air::Directive_Expr, out.first
		assert_instance_of Air::Identifier_Expr, out.first.name
		assert_instance_of Air::Circumfix_Expr, out.first.expression
	end

	def test_all_http_methods
		Air::HTTP_VERBS.each do |verb|
			assert_instance_of Air::Route_Expr, Air.parse("#{verb}://path {;}").first
		end
	end

	def test_route_declaration_with_http_method_directives
		refute_raises Air::Invalid_Http_Directive_Handler do
			out = Air.parse 'get://something {;}'
			assert_equal 1, out.count
			assert_instance_of Air::Route_Expr, out.first
			assert_equal 'get', out.first.http_method.value
			assert_equal "something", out.first.path

			# out = Air.parse_code 'put://"book/:id" replace_book {id;}'
			# assert_equal 1, out.count
			# assert_instance_of Air::Route_Expr, out.first
			# assert_equal 'put', out.first.http_method.value
			# assert_equal "book/:id", out.first.path.value
			#
			# out = Air.parse_code 'patch://"thing/:id" update_thing'
			# assert_equal 1, out.count
			# assert_instance_of Air::Route_Expr, out.first
			# assert_equal 'patch', out.first.http_method.value
			# assert_equal "thing/:id", out.first.path.value
		end

		# assert_raises Invalid_Http_Directive_Handler do
		# 	Air.parse_code '#post "whatever/:id" 1234'
		# end

		out = Air.parse '#pretend_method "endpoint" {;}'
		assert_equal 2, out.count
		refute_instance_of Air::Route_Expr, out.first
		assert_instance_of Air::Directive_Expr, out[0]
		assert_instance_of Air::Func_Expr, out[1]
	end

	def test_empty_html_element_expression
		out = Air.parse '<element> {}'
		assert_equal 1, out.count

		assert_instance_of Air::Html_Element_Expr, out.first
		assert_equal 'element', out.first.element.value
		assert_empty out.first.expressions
	end

	def test_simple_html_element_expression
		out = Air.parse "<My_Div> {
			element = 'div'

			id = 'my_div'
			class = 'my_class'
			data_something = 'some data attribute'

			render {;
				'Text content of this div'
			}
		}"
		assert_equal 1, out.count
		assert_instance_of Air::Html_Element_Expr, out.first
		assert_equal 'My_Div', out.first.element.value

		assert_equal 5, out.first.expressions.count
	end
end
