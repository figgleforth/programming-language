module Ruby_Proxies
	...
	# (Code::Integer < Code::Number) doesn't inherit it and would otherwise report nil, breaking
	...
	# For a Code method with multiple param-typed declarations (`find_table (struct: Struct;)`, `find_table (name: String;)`) --
	# Code itself doesn't support overload dispatch (a repeated name just overwrites the earlier declaration), so only declare
