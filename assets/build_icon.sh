...
#
# Usage:
#   assets/build_icon.sh                       Rebuild assets/icon.icns from assets/icon@2x.svg
...
SRC="${1:-$REPO_ROOT/assets/icon@2x.svg}"
...
mkdir -p "$ICONSET"
# 512 is the largest slot iconutil accepts; its @2x (1024px) covers Retina,
...
for sz in 16 32 128 256 512; do
	...
	d2=$((sz * 2))
	sips -s format png -z "$d2" "$d2" "$SRC" --out "$ICONSET/icon_${sz}x${sz}@2x.png" >/dev/null
done
