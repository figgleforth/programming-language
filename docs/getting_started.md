...
2. **Add an AST node** — a new `Code::Foo_Expr < Expression` in [`expressions.rb`](../source/proxies/expressions.rb). Only add `attr_accessor`s for what's structurally new; `value`/`type`/`l0..c1`/`source_file` are inherited.
